...
def run_crewai(ticket_id: int, prompt_id: int = 1, model: Model = get_default_model()):
    ticket_event = get_ticket_by_id(ticket_id)
    logger.info(f"CREWAI: Processing ticket event: {ticket_event.id}")
    logger.info(f"CREWAI: Using model: {model.provider} - {model.name}")

    ticket_event = process_ticket_event(ticket_event, prompt_id, model)
    if ticket_event:
        logger.info(f"CREWAI: {ticket_event.model_dump_json(indent=2)}")
