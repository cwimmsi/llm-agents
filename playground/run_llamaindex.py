...
def run_llamaindex(ticket_id: int, model: Model = get_default_model()):
    ticket_event = get_ticket_by_id(ticket_id)
    logger.info(f"LLAMAINDEX: Processing ticket event: {ticket_event.id}")
    logger.info(
        f"LLAMAINDEX: Using model: {model.get_provider_value()}/{model.get_name_value()}"
    )

    ticket_event = process_ticket_event(ticket_event, model)
    if ticket_event.is_classified:
        logger.debug(f"LLAMAINDEX: {ticket_event.model_dump_json(indent=2)}")
    else:
        logger.warning(f"LLAMAINDEX: Ticket event not classified.")
