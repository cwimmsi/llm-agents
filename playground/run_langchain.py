import asyncio
from app.langchain.langchain_agent import classify_ticket
...
def run_langchain(ticket_id: int, model: Model = get_default_model()):
    ticket_event = get_ticket_by_id(ticket_id)
    logger.info(f"LANGCHAIN: Processing ticket event: {ticket_event.id}")
    logger.info(
        f"LANGCHAIN: Using model: {model.get_provider_value()}/{model.get_name_value()}"
    )

    ticket_event = process_ticket_event(ticket_event, model)
    if ticket_event.is_classified:
        logger.debug(f"LANGCHAIN: {ticket_event.model_dump_json(indent=2)}")
    else:
        logger.warning(f"LANGCHAIN: Ticket event not classified.")

    evaluate_classification(
        ticket_event.classification, get_expected_classification(ticket_id)
    )

    logger.info(f"LANGCHAIN: Ticket processing completed.")
